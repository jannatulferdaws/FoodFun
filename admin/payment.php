...
<body>
   ...
   ?>
    <div class="wrapper">
        ...
        <?php
          ...
          {
            $sql = "SELECT * FROM `Product` WHERE `product`.`Id` = {$_POST['id']}";
        ...
        <form action="payment.php" method="POST">
            ...
            <div class="input-group">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="input-box">
                            <input type="text" placeholder="Food Name" name="foodname" value="<?php echo $row['Product_Name']; ?>" class="name" required>
  ...
  if(isset($_POST['payment']))
  {
    ...
    $fdiscount=(int)$discount;

    if($fdiscount>0)
    {
      $total_amount=($fprice*$fquantity*$discount)/100;
    }
    else {
    ...
    $sql="INSERT INTO `paymentinfo`( `full_name`, `nick_name`, `email`, `phnNo`, `foodName`, `quantity`, `price`, `gender`, `payment_method`, `card_no`, `card_cvc`, `mobileBankingNo`, `payment_date`, `address`) VALUES ('$fname','$Nname','$email','$phn','$foodName','$quantity','$price','$gender','$pay','$cardNo','$cvc','$BRno','$date','$address')";
  ...
  $sql2="INSERT INTO `orderinfo`( `Name`, `Contact`, `Taka`, `Order_Received`, `Delivery_Time`, `Status`, `item`) VALUES ('$name','$phn','$total_amount','$date','$date','$status','$foodName')";
